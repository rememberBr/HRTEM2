...
#滑动窗口尺寸
elementsize = Crystals.args.elementsize
...
for i in range(len(imgpath)):
    ...
    if image is None:
        ...
        continue
    loadings, factors = NMF(image, elementsize, step)
    # pdb.set_trace()
    #计算滑动窗口的数量
    if  not correction:
        Crystals.args.img_num_w, Crystals.args.img_num_h, img_num_all = imgSplit(Crystals.args, image, elementsize, step)
        history["img_num_w"] = Crystals.args.img_num_w
        history["img_num_h"] = Crystals.args.img_num_h
        correction = True

    imgs_pre = load_data(factors)
    ...
    for k, v in inference_res[path].items():
        resImg = show(loadings[k], image, step)
        cv2.imwrite(os.path.join(Crystals.args.respath, os.path.basename(path).split(".")[0] +"_res_" + str(k)+'_'+ list(v.keys())[0]+'.png'), resImg)
        cv2.imwrite(os.path.join(Crystals.args.respath, os.path.basename(path).split(".")[0] +"_loading_" + str(k)+'_'+ list(v.keys())[0]+'.png'), loadings[k]*255)
        cv2.imwrite(os.path.join(Crystals.args.respath, os.path.basename(path).split(".")[0] +"_factor_" + str(k)+'_'+ list(v.keys())[0]+'.png'), factors[k])
...
print(' finished '+ os.path.join(os.getcwd(), Crystals.args.respath))